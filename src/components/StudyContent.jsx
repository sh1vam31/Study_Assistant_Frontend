...
function StudyContent({ data }) {
  ...
  return (
    <div className="mt-8 space-y-6 animate-fade-in-up">
      {/* Topic Header */}
      <div className="glass-card-strong rounded-2xl p-6 backdrop-blur-xl border border-white/20">
        <div className="flex items-center justify-between">
          ...
          {data.wikipediaUrl && (
            <a
              ...
              className="inline-flex items-center gap-2 bg-white/10 hover:bg-white/20 backdrop-blur-sm px-3 py-1.5 rounded-lg transition-all duration-200 text-xs font-medium text-blue-200"
            >
              <span>📚</span>
              <span>Wikipedia</span>
            </a>
      ...
      <div className="glass-card-strong rounded-2xl p-6 backdrop-blur-xl border border-white/20">
        ...
        <ul className="space-y-3">
          {data.summary.map((point, i) => (
            <li key={i} className="flex items-start gap-3 p-3 bg-white/5 hover:bg-white/10 rounded-xl transition-all duration-200">
              <span className="flex-shrink-0 w-6 h-6 bg-blue-500/50 text-white rounded-lg flex items-center justify-center text-xs font-bold">
                {i + 1}
              </span>
              <span className="text-blue-100 leading-relaxed text-sm pt-0.5">{point}</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className="glass-card-strong rounded-2xl p-6 backdrop-blur-xl border border-white/20">
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3">
          {!showResults ? (
            ...
            <>
              <div className="flex-1 bg-green-500/20 border border-green-400/50 p-4 rounded-xl">
                <div className="flex items-center justify-between">
                  <div>
              ...
              <button
                onClick={resetQuiz}
                className="bg-white/10 hover:bg-white/20 text-white font-medium py-3 px-4 rounded-xl transition-all duration-300 text-sm"
              >
                🔄 Retry
              </button>
            </>
      ...
      {/* Study Tip Section */}
      <div className="glass-card-strong rounded-2xl p-6 backdrop-blur-xl border border-yellow-400/30">
        ...
        <p className="text-blue-100 leading-relaxed text-sm">{data.studyTip}</p>
      </div>
        ...
        <div className="glass-card-strong rounded-2xl p-6 backdrop-blur-xl border border-white/20">
          ...
          <div className="bg-blue-500/20 border border-blue-400/30 p-4 rounded-xl">
            ...
            <div className="flex items-start gap-2">
              <span className="text-lg">📖</span>
              <div>
                <p className="text-xs text-blue-200 mb-1">Explanation</p>
